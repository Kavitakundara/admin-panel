...
if (isset($_POST['update_manager'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Dealer updated successfully'); window.location.href='dealer-view.php';</script>";
    } else {
...
if (isset($_GET['delete_id'])) {
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Dealer deleted successfully'); window.location.href='dealer-view.php';</script>";
    } else {
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td><?php echo htmlspecialchars(!empty($manager['created_by']) ? $manager['created_by'] : 'Super Admin', ENT_QUOTES, 'UTF-8'); ?>
                </td>
...
<td class="mng-dow">
    ...
    if (!empty($manager['document'])): 
        foreach ($documents as $doc): 
            $doc = trim($doc);
            $displayName = (strlen($doc) > 10) ? substr($doc, 0, 10) . '...' : $doc; // Show first 10 characters with "..."
    ?>
    <a href="document/<?= htmlspecialchars($doc); ?>" download>
    ...
    <?php endif; ?>
                ...
                <td style="display:flex">
                    ...
                    <!-- Edit Button -->
                    <form method="post" action="dealer-view.php"> <input type="hidden" name="id"
                            value="<?php echo $manager['id']; ?>">
                        <input type="submit" class="edit-btn" name="edit_manager" value="Edit" />
                    </form>
                    <a href=" manager-detail.php?id=<?php echo $manager['id']; ?>"
